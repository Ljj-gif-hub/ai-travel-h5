...
- **Agent 地图页**：高德地图 + 可拖拽抽屉联动（收放时地图无极缩放）、收起摘要卡、景点标记去重叠、行程调整/保存
...
│   │   ├── AgentMapView.vue          # 🆕 Agent 地图页（高德 + 抽屉缩放 + 摘要卡 + 保存）
│   │   ├── AgentPlannerView.vue      # 🆕 Agent 规划入口
│   │   ├── TripMapView.vue           # 🆕 行程地图（高德/百度/Leaflet 多供应商）
│   │   └── ...                       # 其他子页面
