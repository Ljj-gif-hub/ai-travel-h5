<script setup>
/**
 * OldPlannerView.vue — AI Agent 智能规划（旧版列表式表单）
 *
 * 【保留版】用户要求旧规划不删除，改为可切换。此文件是原 AgentPlannerView 的完整备份，
 * 由新版图标页右上角「回旧版」进入，本页右上角「回新版」返回。逻辑与后端 query 完全未动。
 */
...
defineOptions({ name: 'OldPlannerView' })
...
const goNew = () => { router.push('/agent-planner') }
</script>
...
<template>
  <div class="planner-page">
    ...
    <div class="planner-nav">
      ...
      <button class="nav-quick-fill" @click="goNew">{{ t('agent.backToNew') }}</button>
