...
const fetchMapProvider = async () => {
  try {
    const result = await tripNewApi.getMapConfig()
    if (result.code === 0 && result.data) {
      return result.data.provider || 'amap'
    }
  } catch (e) {
    console.warn('[地图] 获取地图配置失败，默认使用高德地图', e)
  }
  return 'amap'
}
...
/**
 * 加载 Leaflet 免费地图（百度/高德均不可用时的降级方案）
...
/**
 * 加载高德地图 JS API v2.0（通过后端代理）
 * 后端根据 map.provider 配置决定返回哪个 SDK
 */
const loadAmapSDK = () => {
  return new Promise((resolve) => {
    if (window.AMap) { resolve('amap'); return }
    if (amapLoaded) { resolve(window.AMap ? 'amap' : 'unavailable'); return }
    if (window._amapLoading) {
      const check = setInterval(() => {
        if (window.AMap) { clearInterval(check); resolve('amap'); return }
        if (window._amapUnavailable || amapLoaded) { clearInterval(check); resolve('unavailable'); return }
      }, 200)
      return
    }
    ...
    script.onload = () => {
      ...
      const check = setInterval(() => {
        ...
        if (window._amapUnavailable || retries++ > 30) {
          clearInterval(check)
          window._amapLoading = false
          amapLoaded = true
          resolve('unavailable')
...
const initMap = async (centerCity) => {
  ...
  try {
    if (configProvider === 'amap') {
      // 优先尝试高德
      const provider = await loadAmapSDK()
      if (provider === 'amap') {
        mapProvider = 'amap'
        await initAmapMap(centerCity)
        return
      }
      // 高德不可用 → 尝试百度 → Leaflet
      console.log('[地图] 高德不可用，尝试百度地图')
      const baiduResult = await loadBaiduMapSDK()
      if (baiduResult === 'baidu') {
        mapProvider = 'baidu'
      ...
      // 百度不可用 → 尝试高德
      console.log('[地图] 百度不可用，尝试高德地图')
      const amapResult = await loadAmapSDK()
      if (amapResult === 'amap') {
        mapProvider = 'amap'
        await initAmapMap(centerCity)
        return
...
const initBaiduMap = async (centerCity) => {
  ...
  console.log('[地图] 百度地图 GL 初始化完成:', centerCity, center)
}

/**
 * Leaflet 地图初始化（免费 OpenStreetMap 瓦片）
 */
const initLeafletMap = async (centerCity, L) => {
  const center = await getCityCenter(centerCity)
  ...
  await addCityLabelLeaflet(centerCity, L)
  console.log('[地图] Leaflet + OSM 初始化完成:', centerCity, center)
}

/**
 * 城市中心坐标（常见旅游城市硬编码 + API 动态查询）
...
const getCityCenter = async (city) => {
  ...
  const known = {
    ...
    '香港': { lat: 22.280, lng: 114.170 },
    '澳门': { lat: 22.198, lng: 113.549 },
    '台北': { lat: 25.033, lng: 121.565 },
    '天津': { lat: 39.085, lng: 117.200 },
    '苏州': { lat: 31.299, lng: 120.585 },
    '长沙': { lat: 28.228, lng: 112.939 },
    '青岛': { lat: 36.067, lng: 120.383 },
    '大连': { lat: 38.914, lng: 121.615 },
    '昆明': { lat: 25.039, lng: 102.712 },
    '郑州': { lat: 34.747, lng: 113.625 },
    '福州': { lat: 26.074, lng: 119.296 },
    '合肥': { lat: 31.821, lng: 117.227 },
    '贵阳': { lat: 26.647, lng: 106.630 },
    '哈尔滨': { lat: 45.803, lng: 126.535 },
    '沈阳': { lat: 41.806, lng: 123.432 },
    '拉萨': { lat: 29.650, lng: 91.117 },
    '乌鲁木齐': { lat: 43.826, lng: 87.617 },
    '桂林': { lat: 25.274, lng: 110.290 },
    '大理': { lat: 25.607, lng: 100.268 },
    '丽江': { lat: 26.872, lng: 100.230 },
    '张家界': { lat: 29.117, lng: 110.478 },
    '黄山': { lat: 29.715, lng: 118.337 },
    '海口': { lat: 20.017, lng: 110.349 },
    '珠海': { lat: 22.271, lng: 113.577 },
    '南宁': { lat: 22.817, lng: 108.366 },
    '呼和浩特': { lat: 40.842, lng: 111.749 },
    '西宁': { lat: 36.617, lng: 101.778 },
    '银川': { lat: 38.487, lng: 106.231 },
    '兰州': { lat: 36.061, lng: 103.834 },
    '南昌': { lat: 28.683, lng: 115.858 },
    '济南': { lat: 36.651, lng: 116.985 },
    '太原': { lat: 37.871, lng: 112.549 },
    '石家庄': { lat: 38.042, lng: 114.515 },
    '长春': { lat: 43.817, lng: 125.324 },
    '巴黎': { lat: 48.857, lng: 2.352 },
    '东京': { lat: 35.676, lng: 139.650 },
    '伦敦': { lat: 51.507, lng: -0.128 },
    '纽约': { lat: 40.713, lng: -74.006 },
    // 亚洲
    '大阪': { lat: 34.694, lng: 135.502 }, '京都': { lat: 35.012, lng: 135.768 },
    '首尔': { lat: 37.567, lng: 126.978 }, '釜山': { lat: 35.180, lng: 129.076 },
    '曼谷': { lat: 13.754, lng: 100.501 }, '清迈': { lat: 18.788, lng: 98.985 },
    '新加坡': { lat: 1.352, lng: 103.820 }, '吉隆坡': { lat: 3.140, lng: 101.687 },
    '巴厘岛': { lat: -8.340, lng: 115.092 }, '普吉岛': { lat: 7.880, lng: 98.392 },
    '迪拜': { lat: 25.204, lng: 55.271 }, '马尔代夫': { lat: 4.175, lng: 73.509 },
    '胡志明市': { lat: 10.823, lng: 106.630 }, '河内': { lat: 21.028, lng: 105.854 },
    // 欧洲
    '罗马': { lat: 41.903, lng: 12.496 }, '米兰': { lat: 45.464, lng: 9.190 },
    '威尼斯': { lat: 45.441, lng: 12.315 }, '佛罗伦萨': { lat: 43.770, lng: 11.258 },
    '巴塞罗那': { lat: 41.387, lng: 2.169 }, '马德里': { lat: 40.417, lng: -3.704 },
    '布拉格': { lat: 50.076, lng: 14.438 }, '维也纳': { lat: 48.208, lng: 16.373 },
    '苏黎世': { lat: 47.377, lng: 8.540 }, '阿姆斯特丹': { lat: 52.368, lng: 4.904 },
    '柏林': { lat: 52.520, lng: 13.405 }, '慕尼黑': { lat: 48.135, lng: 11.582 },
    '雅典': { lat: 37.984, lng: 23.728 }, '圣托里尼': { lat: 36.393, lng: 25.462 },
    '布达佩斯': { lat: 47.498, lng: 19.040 }, '莫斯科': { lat: 55.756, lng: 37.617 },
    '圣彼得堡': { lat: 59.934, lng: 30.336 }, '伊斯坦布尔': { lat: 41.008, lng: 28.978 },
    '雷克雅未克': { lat: 64.147, lng: -21.940 }, '冰岛': { lat: 64.147, lng: -21.940 },
    // 北美
    '洛杉矶': { lat: 34.054, lng: -118.243 }, '旧金山': { lat: 37.775, lng: -122.419 },
    '芝加哥': { lat: 41.878, lng: -87.630 }, '华盛顿': { lat: 38.907, lng: -77.037 },
    '波士顿': { lat: 42.360, lng: -71.059 }, '西雅图': { lat: 47.606, lng: -122.332 },
    '迈阿密': { lat: 25.762, lng: -80.192 }, '拉斯维加斯': { lat: 36.170, lng: -115.140 },
    '多伦多': { lat: 43.653, lng: -79.383 }, '温哥华': { lat: 49.283, lng: -123.121 },
    // 大洋洲
    '悉尼': { lat: -33.869, lng: 151.209 }, '墨尔本': { lat: -37.814, lng: 144.963 },
    '奥克兰': { lat: -36.848, lng: 174.763 }, '皇后镇': { lat: -45.031, lng: 168.663 },
  }
  if (known[clean]) return known[clean]

  // 未命中 → 调后端地理编码 API
  try {
    const resp = await tripNewApi.getMapConfig().then(() => {}) // ensure config loaded
    const result = await fetch(`/api/map/geocode?address=${encodeURIComponent(clean)}`)
    const json = await result.json()
    if (json.code === 0 && json.data && json.data.length === 2) {
      const coords = { lat: json.data[0], lng: json.data[1] }
      console.log('[地图] 地理编码成功:', clean, '→', coords)
      return coords
    }
  } catch (e) {
    console.warn('[地图] 地理编码失败:', clean, e)
  }
  // 最终兜底
 ...
 * 高德地图初始化
 */
const initAmapMap = async (centerCity) => {
  const center = await getCityCenter(centerCity)
  if (mapInstance && mapProvider === 'amap') {
  ...
  mapInstance = new window.AMap.Map('trip-bmap-container', {
  ...
  addCityLabelAmap(centerCity, center)
  console.log('[地图] 高德地图 JS API v2.0 初始化完成:', centerCity, center)
}
...
/**
 * 在地图上添加标记点（兼容百度地图、高德地图和 Leaflet）
 */
const addMarker = (lat, lng, name, type = 'attraction') => {
  ...
  if (mapProvider === 'baidu') {
    ...
    addMarkerAmap(lat, lng, name, color, size)
...
/** 高德地图标记 */
const addMarkerAmap = (lat, lng, name, color, size) => {
  if (!window.AMap) return
  // 高德坐标格式 [lng, lat]
  const marker = new window.AMap.Marker({
    position: [lng, lat],
    offset: new window.AMap.Pixel(-size, -size),
    content: `<div style="width:${size*2+4}px;height:${size*2+4}px;border-radius:50%;background:${color};border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,0.25)"></div>`,
    title: name,
  })
  ...
  mapInstance.add(label)
  mapMarkers.push({ marker, label, provider: 'amap' })
}
...
const clearMapMarkers = () => {
  ...
  mapMarkers.forEach(({ marker, label, provider }) => {
    if (provider === 'baidu') {
      ...
      try { mapInstance.remove(marker) } catch (e) {}
      if (label) try { mapInstance.remove(label) } catch (e) {}
