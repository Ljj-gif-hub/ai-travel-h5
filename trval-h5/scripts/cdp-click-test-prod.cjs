...
async function main() {
  ...
  await send('Page.navigate', { url: 'http://8.148.223.54/#/login' })
  await sleep(3000)
  console.log('step1 href:', await evalJs('location.href'))
  await waitFor(`location.origin === 'http://8.148.223.54' && document.readyState === 'complete'`, 15000)
  await sleep(2000)
  ...
  // 4. 打开 agent-planner
  await send('Page.navigate', { url: 'http://8.148.223.54/#/agent-planner' })
  await sleep(3000)
